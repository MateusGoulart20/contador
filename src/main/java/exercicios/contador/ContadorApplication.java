...
//import exercicios.contador.Tarefa;
...
public class ContadorApplication implements CommandLineRunner {
	...
	public void salivadas(){
		...
		retornoOptionalInteger =  numeros1.stream()
				...
				.findFirst();
		if ( retornoOptionalInteger.isPresent() ) System.out.println(retornoOptionalInteger.get());
		...
		Optional<String> retorno = nomes.stream()
				.reduce( (p1, p2) -> p1 +", "+ p2 );
		if ( retorno.isPresent() ) System.out.println(retorno.get());
	...
	public void lambidas() {
		...
		List<Integer> numerosPrimos = listaDeNumeros.stream()
				...
				.filter(ContadorApplication::ehPrimo) // Filtrar apenas números primos
		...
		List<Produto> outrosProdutos = produtos.stream()
		...
		List<Produto> maisProdutos = produtos.stream()
